...
router.post('/', auth, function(req, res) {
  ...
  query
    .then(pround => {
      thisPRound = pround;
      return Round.findById(req.body.roundid).populate(
        'questionsLists.questlistId'
      );
    })
    .then(round => {
      thisRound = round;
      ...
      if (!round.questionsLists || round.questionsLists.length == 0) {
        return Promise.reject({
          code: config.CODE_ERR_WITH_MESS,
          message: 'dont have question list'
        });
      }
      ...
      if (now > round.endTime) {
        return Promise.reject({
          code: config.CODE_ERR_WITH_MESS,
          message: 'round playing time expire'
        });
      }

      if (thisPRound && !thisRound.isAllowReplay) {
        if (
          PLAYSTATE.PLAYING == thisPRound.status ||
          PLAYSTATE.BEGIN == thisPRound.status
        ) {
          return Promise.reject({
          ...
          return Promise.reject({
            code: config.CODE_ERR_WITH_MESS,
            message: 'you have played this round'
          });
        }
      }
    ...
    .then(preplay => {
      if (!preplay) {
        return Promise.reject({
          code: config.CODE_ERR_WITH_MESS,
          message: 'You must play previous route'
        });
      }

      const questList =
        thisRound.questionsLists[
          Random.generate(0, thisRound.questionsLists.length - 1)
        ].questlistId;

      //pick question from list
      var histories = [];

      QuestionHelper.choose(
        questList.questions,
        histories,
        questList.usingQuestion,
        questList.isRandom,
        thisRound.playTime ? thisRound.playTime : 30
      );
...
router.get('/played/ground/:pid', auth, (req, res) => {
  const query = PlayGround.findById(req.params.pid, '-history').populate(
    'roundid'
  );
  query
    .then(prounds => {
...
router.post('/play', auth, (req, res) => {
  const query = PlayGround.findById(req.body.pid).populate(
    'history.question roundid'
  );
  query
    .then(pround => {
      if (!pround) {
      ...
      if (global.userCurrentPlay.hasOwnProperty(req.decoded._id)) {
        ...
        if (
          PLAYSTATE.PLAYING ==
            global.userCurrentPlay[req.decoded._id].curstatus &&
          !global.userCurrentPlay[req.decoded._id].curPlay.equals(pround._id)
        ) {
      ...
      if (PLAYSTATE.BEGIN == pround.status) {
        ...
        try {
          if (
            !global.userCurrentPlay.hasOwnProperty(req.decoded._id) ||
            !global.userCurrentPlay[req.decoded._id].curPlay.equals(pround._id)
          ) {
            global.userCurrentPlay[pround.player] = {
              ...
              curquestIndex: pround.questIndex,
              history: pround.history,
              auto: pround.roundid.isAutoPlay
                ? pround.roundid.isAutoPlay
                : true,
...
router.get('/scoreboard', (req, res) => {
  const query = Players.find({}).populate('userId', 'isOnline');
  query
    .then(players => {
      res.json({
        code: config.CODE_OK_WITH_MESS,
